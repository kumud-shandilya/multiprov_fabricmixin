...
type UninstallStep struct {
	UninstallArguments `yaml:"fabric"`
}
...
func (m *Mixin) delete(workspace_id string, token string, item_id string) error {
	...
	response, _ := client.R().
		...
		SetAuthToken(token).
		Delete("https://api.fabric.microsoft.com/v1/workspaces/" + workspace_id + "/items/" + item_id)
...
func (m *Mixin) Uninstall(ctx context.Context) error {
	...
	err = m.delete(action.Steps[0].WorkspaceId, action.Steps[0].Token, action.Steps[0].ItemId)
	if err != nil {
		log.Println(err)
		return err
	}
